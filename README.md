...
## Note for Cluster Users (Feb 2026)

The `redist_smc()` calls in this repo previously included three arguments —
`sampling_space`, `ms_params`, and `split_params` — that do not exist in any
released version of the `redist` package (CRAN or GitHub). These were removed
from all templates and individual simulation scripts.

**If you are running the state house (SHD) batch analyses on the cluster**, the
`03_sim_*.R` scripts in each SHD analysis folder were generated by
`analyses/00_generate_shd_analyses.R`. After pulling the latest changes, you
must re-run that generator to overwrite the locally generated scripts with the
fixed versions:

```r
source("analyses/00_generate_shd_analyses.R")
```

Then re-run any simulation scripts that previously failed with an "unused
arguments" error.
