###############################################################################
# Download and prepare data for `OH_shd_2020` analysis
###############################################################################
...
cli_process_start("Downloading files for {.pkg OH_shd_2020}")

path_data <- download_redistricting_file("OH", "data-raw/OH", year = 2020)
...
dir.create(here("data-out/OH_2020"), showWarnings = FALSE, recursive = TRUE)
...
if (!file.exists(here(shp_path))) {
    ...
    # add municipalities
    d_muni <- make_from_baf("OH", "INCPLACE_CDP", "VTD", year = 2020) |>
    ...
    sldl_shp <- st_read(here("data-raw/OH/sldl_2020/tl_2022_39_sldl.shp"), quiet = TRUE) |>
    ...
    oh_shp <- fill_na_enacted(oh_shp, shd_2020)
