###############################################################################
# Download and prepare data for `WI_shd_2020` analysis
###############################################################################
...
cli_process_start("Downloading files for {.pkg WI_shd_2020}")

path_data <- download_redistricting_file("WI", "data-raw/WI", year = 2020)
...
dir.create(here("data-out/WI_2020"), showWarnings = FALSE, recursive = TRUE)
...
if (!file.exists(here(shp_path))) {
    ...
    # add municipalities
    d_muni <- make_from_baf("WI", "INCPLACE_CDP", "VTD", year = 2020) |>
    ...
    sldl_shp <- st_read(here("data-raw/WI/sldl_2020/tl_2022_55_sldl.shp"), quiet = TRUE) |>
    ...
    wi_shp <- fill_na_enacted(wi_shp, shd_2020)
