###############################################################################
# Download and prepare data for `RI_shd_2020` analysis
###############################################################################
...
cli_process_start("Downloading files for {.pkg RI_shd_2020}")

path_data <- download_redistricting_file("RI", "data-raw/RI", year = 2020)
...
dir.create(here("data-out/RI_2020"), showWarnings = FALSE, recursive = TRUE)
...
if (!file.exists(here(shp_path))) {
    ...
    # add municipalities
    d_muni <- make_from_baf("RI", "INCPLACE_CDP", "VTD", year = 2020) |>
    ...
    sldl_shp <- st_read(here("data-raw/RI/sldl_2020/tl_2022_44_sldl.shp"), quiet = TRUE) |>
        st_transform(st_crs(ri_shp))
    ri_shp <- ri_shp |>
        mutate(shd_2020 = as.integer(sldl_shp$SLDLST)[
            geo_match(ri_shp, sldl_shp, method = "area")])

    # fix labeling
    ri_shp$state <- "RI"

    # eliminate empty shapes
    ri_shp <- ri_shp |> filter(!st_is_empty(geometry))
    ...
    ri_shp$adj <- ri_shp$adj |>
        add_edge(suggest_neighbors(ri_shp, ri_shp$adj)$x,
                 suggest_neighbors(ri_shp, ri_shp$adj)$y)

    ri_shp <- ri_shp |>
        fix_geo_assignment(muni)

    # fill any remaining NA enacted values using adjacency (prevents extra district in redist_map)
    ri_shp <- fill_na_enacted(ri_shp, shd_2020)
