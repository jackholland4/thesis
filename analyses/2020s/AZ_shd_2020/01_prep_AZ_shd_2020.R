###############################################################################
# Download and prepare data for `AZ_shd_2020` analysis
###############################################################################
...
cli_process_start("Downloading files for {.pkg AZ_shd_2020}")

path_data <- download_redistricting_file("AZ", "data-raw/AZ", year = 2020)
...
dir.create(here("data-out/AZ_2020"), showWarnings = FALSE, recursive = TRUE)
...
if (!file.exists(here(shp_path))) {
    ...
    # add municipalities
    d_muni <- make_from_baf("AZ", "INCPLACE_CDP", "VTD", year = 2020) |>
    ...
    sldl_shp <- st_read(here("data-raw/AZ/sldl_2020/tl_2022_04_sldl.shp"), quiet = TRUE) |>
    ...
    az_shp <- fill_na_enacted(az_shp, shd_2020)
