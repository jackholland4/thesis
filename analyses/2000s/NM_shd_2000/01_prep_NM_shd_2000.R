###############################################################################
# Download and prepare data for `NM_shd_2000` analysis
###############################################################################
...
cli_process_start("Downloading files for {.pkg NM_shd_2000}")

path_data <- download_redistricting_file("NM", "data-raw/NM", year = 2000)
...
dir.create(here("data-out/NM_2000"), showWarnings = FALSE, recursive = TRUE)
...
if (!file.exists(here(shp_path))) {
    ...
    sldl_shp <- st_read(here("data-raw/NM/sldl_2000/tl_2010_35_sldl10.shp"), quiet = TRUE) |>
    ...
    nm_shp <- fill_na_enacted(nm_shp, shd_2000)
