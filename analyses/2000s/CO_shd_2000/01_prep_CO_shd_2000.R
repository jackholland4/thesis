###############################################################################
# Download and prepare data for `CO_shd_2000` analysis
###############################################################################
...
cli_process_start("Downloading files for {.pkg CO_shd_2000}")

path_data <- download_redistricting_file("CO", "data-raw/CO", year = 2000)
...
dir.create(here("data-out/CO_2000"), showWarnings = FALSE, recursive = TRUE)
...
if (!file.exists(here(shp_path))) {
    ...
    sldl_shp <- st_read(here("data-raw/CO/sldl_2000/tl_2010_08_sldl10.shp"), quiet = TRUE) |>
    ...
    co_shp <- fill_na_enacted(co_shp, shd_2000)
