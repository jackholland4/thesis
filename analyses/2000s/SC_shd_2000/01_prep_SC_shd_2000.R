###############################################################################
# Download and prepare data for `SC_shd_2000` analysis
###############################################################################
...
cli_process_start("Downloading files for {.pkg SC_shd_2000}")

path_data <- download_redistricting_file("SC", "data-raw/SC", year = 2000)
...
dir.create(here("data-out/SC_2000"), showWarnings = FALSE, recursive = TRUE)
...
if (!file.exists(here(shp_path))) {
    ...
    sldl_shp <- st_read(here("data-raw/SC/sldl_2000/tl_2010_45_sldl10.shp"), quiet = TRUE) |>
    ...
    sc_shp <- fill_na_enacted(sc_shp, shd_2000)
