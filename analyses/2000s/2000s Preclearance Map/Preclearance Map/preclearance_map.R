###############################################################################
# Preclearance Map: Section 5 states with enacted 2000 state senate districts
###############################################################################
...
# Note: TX data not available for 2000 cycle
preclearance_files <- list(
    AK = here("dataverse_files/AK_Sen 2000/tl_2010_02_sldu00.shp"),
    AL = here("dataverse_files/AL_Sen 2000/tl_2010_01_sldu00.shp"),
    AZ = here("dataverse_files/AZ_Sen 2000/tl_2010_04_sldu00.shp"),
    GA = here("dataverse_files/GA_Sen 2000/tl_2010_13_sldu00.shp"),
    LA = here("dataverse_files/LA_Sen 2000/tl_2010_22_sldu00.shp"),
    MS = here("dataverse_files/MS_Sen 2000/tl_2010_28_sldu00.shp"),
    SC = here("dataverse_files/SC_Sen 2000/tl_2010_45_sldu00.shp"),
    VA = here("dataverse_files/VA_Sen 2000/tl_2010_51_sldu00.shp")
)
...
# All 9 fully covered states highlighted even though TX district data unavailable
preclearance_fips <- c(
    "02",  # AK
    "01",  # AL
    "04",  # AZ
    ...
    "28",  # MS
    "45",  # SC
    "48",  # TX (outline only — no district data for 2000)
    "51"   # VA
)
...
p <- ggplot() +
    ...
    geom_sf(data = ak_districts_inset,
            fill = highlight_fill, color = "white", linewidth = 0.15) +
    labs(title = "Section 5 Preclearance States \u2014 Enacted 2000 State Senate Districts") +
    theme_void() +
    theme(
...
# -- Save ---------------------------------------------------------------------
out_path <- file.path(here(), "analyses/2000s/2000s Preclearance Map/Preclearance Map/preclearance_map.png")
