###############################################################################
# Download and prepare data for `NY_shd_2000` analysis
###############################################################################
...
cli_process_start("Downloading files for {.pkg NY_shd_2000}")
...
dir.create(here("data-out/NY_2000"), showWarnings = FALSE, recursive = TRUE)
...
if (!file.exists(here(shp_path))) {
    ...
    sldl_shp <- st_read(here("data-raw/NY/sldl_2000/tl_2010_36_sldl10.shp"), quiet = TRUE) |>
        st_transform(st_crs(ny_shp))
    ny_shp <- ny_shp |>
        mutate(shd_2000 = as.integer(sldl_shp$SLDLST10)[
            geo_match(ny_shp, sldl_shp, method = "area")])

    # fix labeling
    ny_shp$state <- "NY"

    # eliminate empty shapes
    ny_shp <- ny_shp |> filter(!st_is_empty(geometry))
    ...
    ny_shp$adj <- ny_shp$adj |>
        add_edge(suggest_neighbors(ny_shp, ny_shp$adj)$x,
                 suggest_neighbors(ny_shp, ny_shp$adj)$y)

    # fill any remaining NA enacted values using adjacency (prevents extra district in redist_map)
    ny_shp <- fill_na_enacted(ny_shp, shd_2000)
