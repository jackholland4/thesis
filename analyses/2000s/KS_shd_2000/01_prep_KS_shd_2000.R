###############################################################################
# Download and prepare data for `KS_shd_2000` analysis
###############################################################################
...
cli_process_start("Downloading files for {.pkg KS_shd_2000}")

path_data <- download_redistricting_file("KS", "data-raw/KS", year = 2000)
...
dir.create(here("data-out/KS_2000"), showWarnings = FALSE, recursive = TRUE)
...
if (!file.exists(here(shp_path))) {
    ...
    sldl_shp <- st_read(here("data-raw/KS/sldl_2000/tl_2010_20_sldl10.shp"), quiet = TRUE) |>
        st_transform(st_crs(ks_shp))
    ks_shp <- ks_shp |>
        mutate(shd_2000 = as.integer(sldl_shp$SLDLST10)[
            geo_match(ks_shp, sldl_shp, method = "area")])

    # fix labeling
    ks_shp$state <- "KS"

    # eliminate empty shapes
    ks_shp <- ks_shp |> filter(!st_is_empty(geometry))
    ...
    ks_shp$adj <- ks_shp$adj |>
        add_edge(suggest_neighbors(ks_shp, ks_shp$adj)$x,
                 suggest_neighbors(ks_shp, ks_shp$adj)$y)

    # fill any remaining NA enacted values using adjacency (prevents extra district in redist_map)
    ks_shp <- fill_na_enacted(ks_shp, shd_2000)
