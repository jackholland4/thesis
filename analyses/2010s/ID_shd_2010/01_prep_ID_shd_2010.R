###############################################################################
# Download and prepare data for `ID_shd_2010` analysis
###############################################################################
...
cli_process_start("Downloading files for {.pkg ID_shd_2010}")
...
dir.create(here("data-out/ID_2010"), showWarnings = FALSE, recursive = TRUE)

if (!file.exists(here(shp_path))) {
    cli_process_start("Preparing {.strong ID} shapefile")
    # read in redistricting data
    id_shp <- read_csv(here(path_data)) |>
    ...
    sldl_shp <- st_read(here("data-raw/ID/sldl_2010/tl_2013_16_sldl.shp"), quiet = TRUE) |>
    ...
    id_shp <- fill_na_enacted(id_shp, shd_2010)
