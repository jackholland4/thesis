###############################################################################
# Download and prepare data for `RI_shd_2010` analysis
###############################################################################
...
cli_process_start("Downloading files for {.pkg RI_shd_2010}")
...
dir.create(here("data-out/RI_2010"), showWarnings = FALSE, recursive = TRUE)

if (!file.exists(here(shp_path))) {
    cli_process_start("Preparing {.strong RI} shapefile")
    # read in redistricting data
    ri_shp <- read_csv(here(path_data)) |>
        join_vtd_shapefile(year = 2010) |>
        st_transform(EPSG$RI) |>
        rename_with(function(x) gsub("[0-9.]", "", x), starts_with("GEOID"))

    # add municipalities
    d_muni <- make_from_baf("RI", "INCPLACE_CDP", "VTD", year = 2010) |>
    ...
    sldl_shp <- st_read(here("data-raw/RI/sldl_2010/tl_2013_44_sldl.shp"), quiet = TRUE) |>
    ...
    ri_shp <- fill_na_enacted(ri_shp, shd_2010)
