###############################################################################
# Download and prepare data for `CA_shd_2010` analysis
###############################################################################
...
cli_process_start("Downloading files for {.pkg CA_shd_2010}")

path_data <- download_redistricting_file("CA", "data-raw/CA", year = 2010)
...
dir.create(here("data-out/CA_2010"), showWarnings = FALSE, recursive = TRUE)

if (!file.exists(here(shp_path))) {
    cli_process_start("Preparing {.strong CA} shapefile")
    # read in redistricting data
    ca_shp <- read_csv(here(path_data)) |>
        join_vtd_shapefile(year = 2010) |>
        st_transform(EPSG$CA) |>
        rename_with(function(x) gsub("[0-9.]", "", x), starts_with("GEOID"))

    # add municipalities
    d_muni <- make_from_baf("CA", "INCPLACE_CDP", "VTD", year = 2010) |>
    ...
    sldl_shp <- st_read(here("data-raw/CA/sldl_2010/tl_2013_06_sldl.shp"), quiet = TRUE) |>
    ...
    ca_shp <- fill_na_enacted(ca_shp, shd_2010)
