###############################################################################
# Download and prepare data for `MI_shd_2010` analysis
###############################################################################
...
cli_process_start("Downloading files for {.pkg MI_shd_2010}")

path_data <- download_redistricting_file("MI", "data-raw/MI", year = 2010)
...
dir.create(here("data-out/MI_2010"), showWarnings = FALSE, recursive = TRUE)
...
if (!file.exists(here(shp_path))) {
    ...
    sldl_shp <- st_read(here("data-raw/MI/sldl_2010/tl_2013_26_sldl.shp"), quiet = TRUE) |>
    ...
    mi_shp <- fill_na_enacted(mi_shp, shd_2010)
