###############################################################################
# Download and prepare BLOCK-LEVEL data for `WY_shd_2010` analysis
...
cli_process_start("Building block data for {.pkg WY_shd_2010}")

path_data <- build_block_data("WY", "data-raw/WY", year = 2010)

cli_process_done()

# Compile raw data into a final shapefile for analysis -----
shp_path  <- "data-out/WY_2010/shp_block.rds"
perim_path <- "data-out/WY_2010/perim.rds"
dir.create(here("data-out/WY_2010"), showWarnings = FALSE, recursive = TRUE)

if (!file.exists(here(shp_path))) {
    cli_process_start("Preparing {.strong WY} block shapefile")

    wy_shp <- read_csv(here(path_data), col_types = cols(GEOID10 = "c")) |>
        join_block_shapefile(year = 2010) |>
        st_transform(EPSG$WY) |>
        rename_with(function(x) gsub("[0-9.]", "", x), starts_with("GEOID"))

    # add municipalities from Census 2010 BAF at block level
    baf_10 <- get_baf_10("WY",
        cache_to = here("data-raw/WY/WY_baf_10.rds"))
    ...
    sldl_shp <- st_read(here("data-raw/WY/sldl_2010/tl_2013_56_sldl.shp"), quiet = TRUE) |>
    ...
    wy_shp <- fill_na_enacted(wy_shp, shd_2010)
