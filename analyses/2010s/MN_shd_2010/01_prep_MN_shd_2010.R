###############################################################################
# Download and prepare data for `MN_shd_2010` analysis
###############################################################################
...
cli_process_start("Downloading files for {.pkg MN_shd_2010}")
...
dir.create(here("data-out/MN_2010"), showWarnings = FALSE, recursive = TRUE)

if (!file.exists(here(shp_path))) {
    cli_process_start("Preparing {.strong MN} shapefile")
    # read in redistricting data
    mn_shp <- read_csv(here(path_data)) |>
    ...
    sldl_shp <- st_read(here("data-raw/MN/sldl_2010/tl_2013_27_sldl.shp"), quiet = TRUE) |>
    ...
    mn_shp <- fill_na_enacted(mn_shp, shd_2010)
